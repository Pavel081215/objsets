...
abstract class TweetSet {
   ...
   * Возвращает новый `ТweetSet`, который содержит все элементы этого множества, а
   * новый элемент` tweet` в случае, если он уже не существует в этом наборе.
   *
   ...
   * Возвращает новый `TweetSet` исключающий` tweet`.
   */
  def remove(tweet: Tweet): TweetSet

  /**
   * Tests if `tweet` exists in this `TweetSet`.
   * Тесты, если `Text` существует в этом` TweetSet`.
   */
  def contains(tweet: Tweet): Boolean

  /**
   * This method takes a function and applies it to every element in the set.
   * Этот метод принимает функцию и применяет ее к каждому элементу в наборе.
   */
...
class Empty extends TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  override def mostRetweeted: Nothing = throw new NoSuchElementException
